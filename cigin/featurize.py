...
def get_graph_from_smile(molecule_smile):
    ...
    mol_graph = dgl.graph(data=edge_list, num_nodes=molecule.GetNumAtoms())
    mol_graph.ndata['x'] = torch.from_numpy(np.array(node_features))
    mol_graph.edata['w'] = torch.from_numpy(np.array(edge_features))
    return mol_graph
